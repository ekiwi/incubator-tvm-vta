...
class TwoCycleQueue[T <: Data](
  ...
  // check protocol
  //val deq_next = RegNext(do_deq)
  val enq_next = RegNext(do_enq)
  //assert(!(deq_next && do_deq), "-F- Expecting two cycle behavior") // deq can be 1-cycle shifted by overlapping enq
...
// detected as two-port sync mem interface
//  io.rd_data := mem.read(io.addr, !io.wr_en && io.ch_en)
//  when (io.wr_en && io.ch_en) {
//      mem.write(io.addr, io.wr_data.asUInt)
//  }
