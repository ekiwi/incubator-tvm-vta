...
class AccAluAddOverTest(c: Core, inpSize: Int, wgtSize: Int) extends PeekPokeTester(c) {
  ...
  //for (idx <- 0 until inpSize * wgtSize) {valAcc(idx) = (idx+1)%(1<<p(CoreKey).outBits)}
  ...
  val refResult = valAcc.clone
  for (idx <- 0 until inpSize * wgtSize / 2) {refResult(idx) = (refResult(idx) + refResult(idx + inpSize * wgtSize / 2))%(1<<p(CoreKey).outBits)}
  println ("-D- lod test Gold:")
  for (idx1 <- 0 until inpSize) {
    val start = idx1 * wgtSize
    val end = start + wgtSize
    println (s"""-D- acc load test ${refResult.slice(start, end).mkString(",")} """)
  }
  ...
  def genInstr() = {
    ...
    instCnt += 1
    val uopData = List(
      (accBegin, log2Ceil(p(CoreKey).accMemDepth)), 
      (accBegin + inpSize*wgtSize/batch/blockOut/2, log2Ceil(p(CoreKey).accMemDepth)), 
      (0, 32 - p(CoreKey).accMemDepth * 2))
   memManager.addAsBytes(uops, memIdx = uopCnt*uopBytes, dataBytes = uopBytes, 
                                             memManager.constructData(uopData)._1)
    memManager.addAsBytes(instr, memIdx = instBytes*instCnt, dataBytes = instBytes, memManager.memInst(
                   xpad_1 = 0,
                   xpad_0 = 0,
                   ypad_1 = 0,
    ...
    instCnt += 1
    require(inpTSize > 1 && (inpTSize % 2 == 0), s"-F- Tests copies the second half of inp size tesnors to the first half. batch=${inpTSize}")
    memManager.addAsBytes(instr, memIdx = instBytes*instCnt, dataBytes = instBytes, memManager.aluInst( 
               empty_1 = 0,      
               alu_imm = 0,      
               alu_use_imm = 0,      
...
class AccAlu4x32AddOver extends GenericTest( "Core", (p:Parameters) => new Core()(p), (c:Core) => new AccAluAddOverTest(c, 8,32))
